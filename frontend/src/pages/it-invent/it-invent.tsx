...
export default function ItInvent() {
  ...
  const handleToExcel = () => {
    axios
      .post('/it-invent/api_inv/report/', { items: shownItems }, { responseType: 'blob' })
  ...
  return (
    <Box p={'8%'} pt={'5vh'} sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap' }}>
      ...
      <Grid container spacing={1} width={'30%'} minWidth={'450px'}>
        ...
        <Grid item xs={4}>
          <Button onClick={() => setShowNotify(!showNotify)} variant="contained" fullWidth sx={{ height: '39px' }} disabled={inventory && items.length ? false : true}>
            Рассылка
          </Button>
        </Grid>
        {/* <Grid item xs={3}>
