...
export const reports: Array<Report> = [
    ...
    { name: 'По местам', label: "Отчет по рабочит местам", filter: new Map([['Location', ['!EDP']]]) },
    {
        ...
        filter: new Map([
            ['Type', ["BARCODE HANDHELD SCANNER", "BARCODE RADIO BASE", "BARCODE RADIO SCANNER", "BARCODE TABLE SCANNER", "CR CASH DRAW", "CR CUSTOMER MONITOR", "CR FISCAL REGISTER", "CR MONITOR", "CR SYSTEM BASE"]],
    ...
    },
    {
        ...
        filter: new Map([
            ...
            ["Model", (["DS4308-HD7U2100AZW", "SYMBOL LS2208", "PSC HS1250", "PSC MAGELLAN 8200", "DVD-RW", "CHS9000 (without ethernet ports)", "CRD9000-411CR (w pwr supply & DC cbl)", "SIEMENS OptiPoint 500 ADVANCE", "SIEMENS OPTIPOINT 500", "3COM SWITCH", "HP EliteDesk 800 G1 SFF", "HP t510 Smart Zero ES WF Flex TC", "Zebra ZXP3", "HP t5335z"]).map((item) => '!' + item)]]
        )
    },
]
    ...
    if (items && filter &&  filter.size) {
        return items.filter((item: Item) => {
            return ([...filter]).map((one) => {
                ...
                return (yes.length===0  ||  yes.some((attr: string) =>{
                    if (attr.length==0) { return true}
                    else if (attr === "?") { return item[one[0]]?true : false} // if ! than in not include
                    { return ((item[one[0]]?item[one[0]] : '').toLowerCase()).includes(attr.toLowerCase()) }
                })) && !((not).some((attr) => {
                    if (attr.length<=1) {return false}
                    else {return  ((item[one[0]]?item[one[0]]: '').toLowerCase()).includes(attr.slice(1).toLowerCase()) }
                }))
            }).every((condi) => condi);
        })}
    return items
