...
export default function Mobile() {
  ...
  const [action, setAction] = useState<string | null>(null); // make it reducer
  ...
  useEffect(() => {
    ...
    if (querry.length > 3) {
      ...
      axios.post('', { querry: querry }, { signal: signal });
    } else if (action === 'send') {
    }
    setLoading(false);
  }, [querry, action]);
  return (
    <>
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItemss: 'center', padding: 0, margin: 0, height: '100%', width: '100%' }}>
        <Grid container sx={{ boxShadow: 8, height: '86vh', width: '84%' }}>
          <Grid item xs={l}>
